void R15b_Am241Sim_ChangingSourceMaterial() {

  const int n_settings = 2;
  std::string materials[n_settings] = {"Aluminium", "Mylar"};
  ...
  for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
    ...
    if (i_setting > 0) {
      mc_filename << "_" << materials[i_setting];
    }
    mc_filename << ".root";
    ...
    mc_histname.str("");
    mc_histname << "mc_2D_hist_" << materials[i_setting];
