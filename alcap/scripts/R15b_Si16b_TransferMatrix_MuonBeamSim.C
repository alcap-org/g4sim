...
void R15b_Si16b_TransferMatrix_MuonBeamSim() {
  ...
  right_arm.third_exists = false;//true;
  ...
  right_arm.third_k_factor = -1;//1.0;//0.8;//0.8;
  ...
  std::string particles[n_particles] = {"muon"};
  std::string sample_sizes[n_particles] = {"1M"};

  const int n_settings = 1;
  std::string settings[n_settings] = { "" };

  const int n_bin_widths = 1;
  double bin_widths[n_bin_widths] = {1000};//, 0.5};
  std::string BinWs[n_bin_widths] = {"1000keV"};//, "500keV"};
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
    ...
    for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
      ...
      std::string filename = "output/R15b_" + sample_size + "_Geom-P5_muon-beam-sim_chamberAndCollsVacuum.root";
      // if (setting != "") {
      // 	filename += "-" + setting;
      // }
	...
	if ( (*i_arm).two_layer_response != NULL) {
      ...
      R15b_TransferMatrix(filename, outfilename, particle, tgtvolname, max_axis, arms, bin_width);
    }
